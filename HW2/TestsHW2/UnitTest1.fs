...
module ExpressionTests =
    ...
    let ``Evaluate (2 + 3) * 4 should return 20`` () =
        Binary(Multiply, Binary(Add, Number 2.0, Number 3.0), Number 4.0) 
        |> evaluate |> should equal 20.0
    
    [<Test>]
    let ``Evaluate sqrt(16) should return 4`` () =
        Unary(Sqrt, Number 16.0) |> evaluate |> should equal 4.0
    
    [<Test>]
    let ``Evaluate division by zero should throw`` () =
        (fun () -> Binary(Divide, Number 10.0, Number 0.0) |> evaluate |> ignore) 
...
module BinTreeTests =
    ...
    [<Test>]
    let ``TreeMap should double all values`` () =
        let tree = createTestTree ()
        let result = treeMap tree (fun x -> x * 2)
    ...
    [<Test>]
    let ``TreeMap should not mutate original tree`` () =
        let tree = createTestTree ()
        let _ = treeMap tree (fun x -> x * 100)
        areTreesEqual tree (createTestTree ()) |> should equal true
